package eu.digiwhist.worker.pl.raw;
 ...
 * @author Tomas Mrazek
 */
public final class UZPTenderCrawler extends BaseDigiwhistIncrementalFtpCrawler {
    /**
    ...
    protected void crawlSourceForDate(final LocalDate date) {
        final String packageDir = XML_PACKAGES_DIR + date.getYear() + "/";
        ...
        if (date.isBefore(DAILY_PACKAGES_START_DATE)) {
            ...
            createAndPublishMessage(String.format("%1$s%2$d_xml.rar", packageDir, date.getYear()), metaData);
        } else {
