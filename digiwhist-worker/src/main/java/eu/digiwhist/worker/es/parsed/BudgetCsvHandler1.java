...
import static eu.digiwhist.worker.es.parsed.BudgetCsvParser.CAP2;
import static eu.digiwhist.worker.es.parsed.BudgetCsvParser.CAP3;
import static eu.digiwhist.worker.es.parsed.BudgetCsvParser.CSV_TYPE_1;
import static eu.digiwhist.worker.es.parsed.BudgetCsvParser.parseBudget;

/**
 * Budget csv parsing handler.
 */
public final class BudgetCsvHandler1 {

    /**
     * Suppress default constructor for noninstantiability.
     */
    private BudgetCsvHandler1() {
    }
    ...
    public static List<ParsedBudgetItem> parse(final RawData raw, final String year) throws IOException {
        final CSVParser csvParser = CSVParser.parse(raw.getSourceData(),
                CSVFormat.newFormat(',').withHeader(CSV_TYPE_1).withSkipHeaderRecord(true));
        ...
        for (int i = 0; i < budgets.size(); i++) {
            ...
            try {
                ...
                if (budget.size() > 3) {
                    parsedTenders.add(parseBudget(budget, CAP2, year, raw.getSourceFileName() + ":" + i));
                }
