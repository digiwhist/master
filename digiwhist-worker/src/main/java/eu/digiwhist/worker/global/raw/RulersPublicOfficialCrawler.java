...
public final class RulersPublicOfficialCrawler extends BaseHttpCrawler {
    ...
    protected void doWork(final Message message) {
        try {
            ...
            final List<HtmlAnchor> linksToDetails =
                    (List<HtmlAnchor>) page.getByXPath("//a[starts-with(@href, 'rul')]");
            ...
            for (HtmlAnchor linkToDetails : linksToDetails) {
                ...
                @SuppressWarnings("unchecked")
                final List<HtmlAnchor> linksToMinistries =
                        (List<HtmlAnchor>) page.getByXPath("//a[contains(text(), 'Ministries, etc.')]");
                ...
                @SuppressWarnings("unchecked")
                final List<HtmlAnchor> linksToRegions =
                        (List<HtmlAnchor>) page.getByXPath("//a[contains(text(), 'Regions')]");
