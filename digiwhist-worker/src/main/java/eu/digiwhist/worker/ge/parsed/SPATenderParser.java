package eu.digiwhist.worker.ge.parsed;
 ...
 * @author Marek Mikes
 */
public class SPATenderParser extends BaseDigiwhistTenderParser {
    ...
    public final List<ParsedTender> parse(final RawData rawTender) {
        ...
        final String buyerNameTitle = "Procuring entities";

        // get buyer snippet
        Elements buyerLinkElements = JsoupUtils.select("div[id='print_area'] > table > tbody > tr:has(td:containsOwn("
                        + buyerNameTitle + ")) > td:nth-child(2) > a",
                mainSnippet);
        ...
        final boolean includingVat = vatInfo.equals("Including VAT");
        ...
        parsedTender
                ...
                .addBuyer(new ParsedBody()
                        .setName(getFromTableFromMainSnippet(buyerNameTitle, mainSnippet))
    ...
    private String getFromTableFromMainSnippet(final String title, final Document mainSnippet) {
        return JsoupUtils.selectText(
                "div[id='print_area'] > table > tbody > tr:has(td:containsOwn(" + title + ")) > td:nth-child(2)",
                mainSnippet);
    }
    ...
    private List<ParsedCPV> parseTenderCpvs(final Document mainSnippet) {
        ...
        if (!mainCpvAndDescriptionString.isEmpty()) {
    ...
    private String parseAwardDeadlineDuration(final Document mainSnippet) {
        ...
        if (awardDeadlineDuration.equals("Day")) {
