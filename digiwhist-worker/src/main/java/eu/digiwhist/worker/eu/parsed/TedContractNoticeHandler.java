...
public final class TedContractNoticeHandler {
    ...
    public static ParsedTender parse(final ParsedTender parsedTender, final Document document) {
        ...
        //I
        final Element contractingBodyNode = JsoupUtils.selectFirst("CONTRACTING_AUTHORITY_INFORMATION", originNode);
        //II
        final Element contractNode = JsoupUtils.selectFirst("OBJECT_CONTRACT_INFORMATION", originNode);
        //III
        final Element leftiNode = JsoupUtils.selectFirst("LEFTI_CONTRACT", originNode);
        //IV
        final Element procedureNode = JsoupUtils.selectFirst("PROCEDURE_DEFINITION_CONTRACT_NOTICE", originNode);
        //VI
        ...
        String procedureType = TedTenderParserUtils.parseProcedureType(
            JsoupUtils.selectFirst("TYPE_OF_PROCEDURE > TYPE_OF_PROCEDURE_DETAIL_FOR_CONTRACT_NOTICE", procedureNode));

        parsedTender
            .addPublication(parseMainPublication(originNode))
            .addPublications(parseOtherPublications(JsoupUtils.select(
                "PREVIOUS_PUBLICATION_NOTICE_F2, OTHER_PREVIOUS_PUBLICATIONS > OTHER_PREVIOUS_PUBLICATION",
                JsoupUtils.selectFirst(
                    "ADMINISTRATIVE_INFORMATION_CONTRACT_NOTICE > PREVIOUS_PUBLICATION_INFORMATION_NOTICE_F2"
                        + " > PREVIOUS_PUBLICATION_EXISTS_F2", procedureNode))))
            ...
            .setIsCoveredByGpa(
                JsoupUtils.hasAttribute(JsoupUtils.selectFirst("CONTRACT_COVERED_GPA", contractNode), "VALUE", "YES")
                    .toString())
            .setHasLots(
                Boolean.toString(!JsoupUtils.exists("F02_DIVISION_INTO_LOTS > DIV_INTO_LOT_NO", contractNode)))
            .setLots(parseLots(
                JsoupUtils.select("F02_DIVISION_INTO_LOTS > F02_DIV_INTO_LOT_YES > F02_ANNEX_B", contractNode)))
            ...
            .setPersonalRequirements(JsoupUtils.selectText(
                "F02_CONDITIONS_FOR_PARTICIPATION > ECONOMIC_OPERATORS_PERSONAL_SITUATION", leftiNode))
            .setEconomicRequirements(
                JsoupUtils.selectCombinedText("F02_ECONOMIC_FINANCIAL_CAPACITY > *", leftiNode))
