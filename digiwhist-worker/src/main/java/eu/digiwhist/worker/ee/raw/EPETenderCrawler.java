package eu.digiwhist.worker.ee.raw;
 ...
 * @author Marek Mikes
 */
public final class EPETenderCrawler extends BaseDigiwhistIncrementalPagedSourceHttpCrawler {
    ...
    public HtmlPage extractDetailsFromPage(final HtmlPage page) {
        @SuppressWarnings("unchecked") final List<HtmlTableRow> noticeTableRows = (List<HtmlTableRow>) page.getByXPath(
                "//table[@id='tableview']/tbody/tr[starts-with(@id, 'informal')]");
        for (HtmlTableRow noticeTableRow : noticeTableRows) {
            try {
                ...
                final HtmlAnchor noticePermalink = noticePage.getFirstByXPath("//a[@id='GenericLink']");
