 ...
 * @author Marek Mikes
 */
public class HungaryOldDataTenderCleaner extends BaseDatlabTenderCleaner {
    ...
    protected final ParsedTender preProcessParsedItem(final ParsedTender parsedItem) {
        for (ParsedPublication publication : parsedItem.getPublications()) {
            if (publication.getHumanReadableUrl() != null) {
                publication.setHumanReadableUrl(publication.getHumanReadableUrl().replace("mutat", "megtekint"));
                if (!publication.getHumanReadableUrl().endsWith("/")) {
                    publication.setHumanReadableUrl(publication.getHumanReadableUrl() + "/");
                }
            }
        }

        if (parsedItem.getIsElectronicAuction() != null) {
            if (parsedItem.getIsElectronicAuction().equals("1")) {
                parsedItem.setIsFrameworkAgreement(Boolean.TRUE.toString());
            } else if (parsedItem.getIsElectronicAuction().equals("0")) {
            ...
            for (ParsedTenderLot lot : parsedItem.getLots()) {
                if (lot.getBids() != null) {
                    for (ParsedBid bid : lot.getBids()) {
                        if (bid.getIsSubcontracted() != null) {
                            if (bid.getIsSubcontracted().equals("1")) {
                                bid.setIsSubcontracted(Boolean.TRUE.toString());
                            } else if (bid.getIsSubcontracted().equals("0")) {
                                bid.setIsSubcontracted(Boolean.FALSE.toString());
                            }
                        }
                    }
                }
    ...
    protected final void registerSpecificPlugins() {
        ...
        pluginRegistry
                ...
                .registerPlugin("address", new AddressPlugin(getCountryMapping()))
    ...
    /**
     * @return country mapping for cleaning process
     */
    private static Map<Enum, List<String>> getCountryMapping() {
        final Map<Enum, List<String>> mapping = new HashMap<>();

        mapping.put(CountryCode.HU, Collections.singletonList("127"));
