 ...
 * @author Tomas Mrazek
 */
public final class GovUKTenderCrawler extends BaseDatlabIncrementalCrawler {
    ...
    private HtmlPage getResultPageForDate(final HtmlPage page, final LocalDate date) throws IOException {
        ...
        // is necessary to mark off these checkboxes for retrieving all records
        markCheckboxIfNeeded(page.getFirstByXPath("//input[@name='open']"));
        markCheckboxIfNeeded(page.getFirstByXPath("//input[@name='closed']"));
        markCheckboxIfNeeded(page.getFirstByXPath("//input[@name='awarded']"));
    ...
    /**
     * @param checkbox
     *      checkbox to be marked off in case that it isn't
     */
    private void markCheckboxIfNeeded(final HtmlCheckBoxInput checkbox) {
