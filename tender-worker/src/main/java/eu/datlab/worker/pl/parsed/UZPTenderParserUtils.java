...
public final class UZPTenderParserUtils {
    protected static final Logger LOGGER = LoggerFactory.getLogger(UZPTenderParserUtils.class.getName());
    ...
    public static Boolean isEnabled(final String selector, final Element context) {
        ...
        if (Arrays.asList("A", "T", "Y").stream().anyMatch(n -> value.equalsIgnoreCase(n))) {
            return true;
        }
        if (Arrays.asList("N", "F").stream().anyMatch(n -> value.equalsIgnoreCase(n))) {
            return false;
        }

        LOGGER.error("Unknown boolean string '{}'", value);
