...
public final class AOPTenderCrawler extends BasePagedSourceHttpCrawler {
    ...
    public HtmlPage extractDetailsFromPage(final HtmlPage page) {
        final List<HtmlAnchor> tenderDetailpageLinks = page.getByXPath("//table[@id='resultaTable']/tbody/tr/"
        ...
        for (final HtmlAnchor tenderDetailpageLink : tenderDetailpageLinks) {
            try {
                ...
                final List<HtmlElement> tenderFormCells = tenderDetailFramePage.getByXPath("//a[@class='newslinks']/ancestor::td");
                for (final HtmlElement tenderFormCell : tenderFormCells) {
                    final List<HtmlAnchor> tenderFormLinks = tenderFormCell.getByXPath("a");
