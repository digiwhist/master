...
public class JdbcMatchedBodyDAO extends GenericJdbcDAO<MatchedBody> implements MatchedBodyDAO<MatchedBody> {
    ...
    public final Map<String, String> getHashAndGroupIds() {
        ...
        try {
            while (true) {
                PreparedStatement statement = connection.prepareStatement("WITH worker_related as (" +
                        "SELECT modified, data FROM " + getTableWithSchema() + " WHERE ((createdBy = ? AND " +
                        "createdByVersion = ?)" +
                        prepareAdditionalWorkersCondition() + ")" + prepareModifiedAfterCondition(
                        previousDate) +
                        ") " +
                        "SELECT modified, data->>'groupId' as groupId, data->>'hash' as hash, jsonb_array_elements" +
                        "(data->'alternativeHashes')->>'hash' as alternativeHash " +
                        "FROM worker_related order by modified asc limit 100000;");
