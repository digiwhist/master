...
public class JdbcMatchedBodyDAO extends GenericJdbcDAO<MatchedBody> implements MatchedBodyDAO<MatchedBody> {
	...
	public final Map<String, String> getHashAndGroupIds() {
		...
		List<MatchedBody> bodies = getMine(page);
		
		while (bodies != null && !bodies.isEmpty()) {
			for (MatchedBody body : bodies) {
				if (body.getHash() != null) {
					result.put(body.getHash(), body.getGroupId());
				}
					
				if (body.getAlternativeHashes() != null) {
					for (WeightedHash hash : body.getAlternativeHashes()) {		
						result.put(hash.getHash(), body.getGroupId());
					}
				}
			}
