 ...
 *
 * @param <T> item type to be mastered
 * @param <V> master item type
 * @param <U> root items - context
 */
public final class CpvPlugin<T extends BaseMatchedTenderLot & MasterablePart, V extends BaseMasterTenderLot, U>
    ...
    public V master(final List<T> items, final V finalItem, final List<U> context) {
        ...
        List<CPV> masteredCpvs = new ArrayList<>();
     ...
     * - if more same specific then the last one
     *
     * @param cpv1                first CPV
     * @param cpv2                second CPV
     * @param cpv1PublicationDate publication date of first CPV
     * @param cpv2PublicationDate publication date of second CPV
