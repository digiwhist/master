...
import eu.dl.worker.master.plugin.generic.comparators.CreatedDateComparator;
 ...
 *         context items type
 */
public class LastCreatedPlugin<T extends MasterablePart, V, U> extends GenericMasterPlugin
     ...
     */
    public LastCreatedPlugin(final List<String> fieldNames, final Converter converter) {
        super(fieldNames);
     ...
     */
    public LastCreatedPlugin(final String fieldName, final Converter converter) {
    ...
    public final V master(final List<T> items, final V finalItem, final List<U> context) {
        final List<T> sortedItems = items
            .stream()
            .sorted(new CreatedDateComparator<>())
